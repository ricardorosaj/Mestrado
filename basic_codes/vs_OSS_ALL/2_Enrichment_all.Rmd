...
```{r}
# DEGs
rm_OSS_DEGs <- read.csv("tables/rm_OSSvsOSS_ALL_72h_DEGs.csv")

rm_OxPAPC_DEGs <- read.csv("tables/rm_OxPAPCvsOSS_ALL_72h_DEGs.csv")

rm_CoCl2_DEGs <- read.csv("tables/rm_CoCl2vsOSS_ALL_72h_DEGs.csv")

rm_IL1B_DEGs <- read.csv("tables/rm_IL1BvsOSS_ALL_72h_DEGs.csv")

#LSS_48h_DEGs <- read.csv("tables/LSS_48hvsOSS_ALL_72h_DEGs.csv")

# All genes
rm_OSS_all <- read.csv("tables/rm_OSSvsOSS_ALL_72h_allgenes.csv")

rm_OxPAPC_all <- read.csv("tables/rm_OxPAPCvsOSS_ALL_72h_allgenes.csv")

rm_CoCl2_all <- read.csv("tables/rm_CoCl2vsOSS_ALL_72h_allgenes.csv")

rm_IL1B_all <- read.csv("tables/rm_IL1BvsOSS_ALL_72h_allgenes.csv")

#LSS_48h_all <- read.csv("tables/LSS_48hvsOSS_ALL_72h_allgenes.csv")
```
...
tiff("images/Dissertação/TIFF/JACCARD_heatmap.tif",width = 30, height = 40, units = 'in', res=600)
...
## Dotplot of enrcihment

```{r fig.height=30, fig.width=12}
dotplot_values <- data.frame(cbind(ht_score,ht_OR))
list_scores <- c(dotplot_values$rm_OSS, dotplot_values$rm_OxPAPC, dotplot_values$rm_CoCl2, dotplot_values$rm_IL1B)
list_or <- c(dotplot_values$OR_OSS, dotplot_values$OR_rm_OxPAPC, dotplot_values$OR_rm_CoCl2, dotplot_values$OR_rm_IL1B)

dotplot_df <- data.frame(Terms = rep(rownames(dotplot_values),4), 
                         Comparisons = rep(c("rm OSS vs OSS+ALL","rm OxPAPC vs OSS+ALL","rm CoCl2 vs OSS+ALL","rm IL1B vs OSS+ALL"), 4,each = length(rownames(dotplot_values))),
                         Score = list_scores,
                         Odds.Ratio = list_or)
...
tiff("images/Dissertação/TIFF/Jaccard_dotplot.tiff",width = 8, height = 30, units = 'in', res=600)
dotplot 
dev.off()
```

## Plot heatmaps for clusters

```{r}
# Select from "groups_path" the pathways from each cluster wanted and plot their heatmap
plot_heatmap_for_selected_clusters <- function(cluster,cluster_name,bool_iguara){
  ...
  genes <- unique(unlist(as.list(strsplit(selected_clusters$Molecules,"\\; |\\;| "))))
  ngenes <- length(genes)
...
tiff("images/Dissertação/TIFF/heatmaps_atualizados/HT_cholesterol.tif", units="in", width=5, height=10, res=600)
plot_heatmap_for_selected_clusters(2,"Cluster b - Cholesterol biosynthesis",FALSE)
dev.off()

tiff("images/Dissertação/TIFF/heatmaps_atualizados/HT_chemotaxis.tif", units="in", width=5, height=10, res=600)
plot_heatmap_for_selected_clusters(3,"Cluster c - Chemotaxis",FALSE)
dev.off()

tiff("images/Dissertação/TIFF/heatmaps_atualizados/HT_mitosis_1.tif", units="in", width=5, height=12, res=600)
plot_heatmap_for_selected_clusters(4,"Cluster d - Mitosis",FALSE)
dev.off()

tiff("images/Dissertação/TIFF/heatmaps_atualizados/HT_mitosis_2.tif", units="in", width=5, height=12, res=600)
plot_heatmap_for_selected_clusters(5,"Cluster e - Mitosis",FALSE)
dev.off()

tiff("images/Dissertação/TIFF/heatmaps_atualizados/HT_ecm.tif", units="in", width=5, height=12, res=600)
plot_heatmap_for_selected_clusters(6,"Cluster f - ECM organization",FALSE)
dev.off()

tiff("images/Dissertação/TIFF/heatmaps_atualizados/HT_cell_nmigration.tif", units="in", width=5, height=12, res=600)
plot_heatmap_for_selected_clusters(7,"Cluster g - Cell migration",FALSE)
dev.off()

tiff("images/Dissertação/TIFF/heatmaps_atualizados/HT_inflammatory_response.tif", units="in", width=5, height=12, res=600)
plot_heatmap_for_selected_clusters(9,"Cluster i - Inflammatory Response",FALSE)
dev.off()

# #If too many genes in heatmap, plot each enriched term
# selected_clusters_inflammation <- groups_path[groups_path$cluster == 2, c("Pathways","Molecules")]
# selected_clusters_angio <- groups_path[groups_path$cluster == 7, c("Pathways","Molecules")]
# 
# plot_heatmap_for_selected_pathways(2,"TNF-alpha Signaling via NF-kB MsigDB",TRUE)
...
```{r}
# # Plot pathways in clusters
plot_heatmap_for_multiple_pathways <- function(cluster,pathways,title,bool_iguara){
  TFs_list <- read.table(file = 'tables/TFs/trrust_rawdata.human.tsv', sep = '\t', header = FALSE)
  TFs_list <- unique(TFs_list$V1)

  selected_clusters <- groups_path[groups_path$cluster == cluster, c("Pathways","Molecules")]
  selected_pathway <- selected_clusters[selected_clusters$Pathways %in% pathways,]
  genes <- unique(unlist(as.list(strsplit(selected_pathway$Molecules,"\\; |\\;| "))))
  ngenes <- length(genes)
  genes_exp <- as.data.frame(matrix(0,nrow = ngenes, ncol = 4, dimnames = list(genes,c("rm OSS","rm OxPAPC","rm CoCl2","rm IL1B"))))
  rm_oss <- rm_OSS_all[rm_OSS_all$SYMBOL %in% genes,c("SYMBOL","logFC")]
  rm_cocl2 <- rm_CoCl2_all[rm_CoCl2_all$SYMBOL %in% genes,c("SYMBOL","logFC")]
  rm_oxpapc <- rm_OxPAPC_all[rm_OxPAPC_all$SYMBOL %in% genes,c("SYMBOL","logFC")]
  rm_il1b <- rm_IL1B_all[rm_IL1B_all$SYMBOL %in% genes,c("SYMBOL","logFC")]
  ...
  col_fun  <- colorRamp2(c(-5, 0, 8), c("blue", "white", "red"))
...
```{r fig.height=10, fig.width=5}
terms <- c("positive regulation of nitric oxide biosynthetic process (GO:0045429)","positive regulation of nitric oxide metabolic process (GO:1904407)","regulation of nitric oxide biosynthetic process (GO:0045428)")
tiff("images/Dissertação/TIFF/heatmaps_atualizados/HT_no_synthesis.tif", units="in", width=5, height=10, res=600)
plot_heatmap_for_multiple_pathways(8,terms,"Cluster h - NO synthesis",FALSE)
dev.off()
```
...
<!-- circos_data <- groups_path %>%  -->
<!--   dplyr::select(Pathways, Molecules, cluster_letters) -->

<!-- # Separate genes in rows and create new data frame -->
<!-- circos_data <- circos_data %>%  -->
<!--   separate_rows(c(Molecules, cluster_letters), sep = ";") %>% -->
<!--   separate_rows(c(Molecules, cluster_letters), sep = " ") %>% -->
<!--   dplyr::select(Pathways, Molecules, cluster_letters)  -->

<!-- # Add FC values for each comparison -->

<!-- OSS_all <- read.csv("tabelas/OSS_vs_LSS_all.csv") -->

<!-- STAT_all <- read.csv("tabelas/STAT_vs_LSS_all.csv") -->



<!-- circos_data <- circos_data %>%  -->
<!--   left_join(OSS_all, by = c("Molecules" = "SYMBOL")) %>%  -->
<!--   dplyr::select(Pathways, Molecules, cluster_letters, logFC) %>%  -->
<!--   dplyr::rename(OSS = logFC) -->

<!-- circos_data <- circos_data %>%  -->
<!--   left_join(STAT_all, by = c("Molecules" = "SYMBOL")) %>%  -->
<!--   dplyr::select(Pathways, Molecules, cluster_letters, OSS, logFC) %>%  -->
<!--   dplyr::rename(STAT = logFC) -->
...
<!-- cluster_b <- circos_data %>%  -->
<!--   filter(cluster_letters == "b") -->

<!-- # heatmap data -->
<!-- heat_b <- cluster_b %>%  -->
<!--   dplyr::select(Molecules, OSS, STAT) -->

<!-- rownames(heat_b) <- NULL -->
<!-- heat_b <- subset(heat_b, !duplicated(Molecules)) -->

<!-- heat_b <- heat_b %>%  -->
<!--   tibble::column_to_rownames(var = "Molecules") %>%  -->
<!--   as.matrix() -->


<!-- # select cluster of interest -->
<!-- cluster_r <- circos_data %>%  -->
<!--   filter(cluster_letters == "r") -->

<!-- # heatmap data -->
<!-- heat_r <- cluster_r %>%  -->
<!--   dplyr::select(Molecules, OSS, STAT) -->

<!-- rownames(heat_r) <- NULL -->
<!-- heat_r <- subset(heat_r, !duplicated(Molecules)) -->
...
<!-- ```{r} -->
<!-- # color -->
<!-- col_fun  <- colorRamp2(c(-6, 0, 6), c("darkblue", "white", "indianred")) -->
...
<!--                     clustering_method_rows = 'ward.D2', -->
<!--                     cell_fun = function(j, i, x, y, width, height, fill) { -->
<!--                     grid.text(sprintf("%.1f", heat_b[i, j]), x, y, gp = gpar(fontsize = 10)) -->
<!--  }) -->
...
<!--                     clustering_method_rows = 'ward.D2', -->
<!--                     cell_fun = function(j, i, x, y, width, height, fill) { -->
<!--                     grid.text(sprintf("%.1f", heat_r[i, j]), x, y, gp = gpar(fontsize = 10)) -->
<!--  }) -->



<!-- ht_list = heatmap_b %v% heatmap_r  -->

<!-- png(file = "Enrichment/Enrichr/cluster_heats.png", width = 6, height = 18, units = "in",res = 300) -->
<!-- ht_list -->
<!-- dev.off() -->
<!-- ``` -->
